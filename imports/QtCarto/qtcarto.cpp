    ...
    m_viewport(nullptr)
{
  ...
  // Set viewport center and zoom level
  size_t level = 16;
  // QcGeoSexagesimalAngle longitude(6, 7, 0);
  // QcGeoSexagesimalAngle latitude(44, 41, 0);
  double longitude =  6.1639;
  double latitude  = 44.6900;
  // mercator x = 686 164  y = 5 572 852
  QcGeoCoordinateWGS84 coordinate(longitude, latitude);
  m_viewport->zoom_at(coordinate.mercator(), level); // 686 162, 5 572 849 m
  // QcGeoCoordinateWGS84 coordinate = m_viewport->wgs84();
  qInfo() << "wgs84 " << coordinate.longitude() << coordinate.latitude();
